...
public:
  ...
  Direction getDirection(movment mov) {
    if (mov.directionX == 1 && mov.directionY == 0)
      return Direction::right;
    if (mov.directionX == -1 && mov.directionY == 0)
      return Direction::left;
    if (mov.directionX == 0 && mov.directionY == 1)
      return Direction::up;
    if (mov.directionX == 0 && mov.directionY == -1)
      return Direction::down;
    if (mov.directionX == 1 && mov.directionY == 1)
      return Direction::diagonalUp;
    if (mov.directionX == -1 && mov.directionY == -1)
      return Direction::diagonalDown;
    if (mov.directionX == 0 && mov.directionY == 0)
  ...
  void rotateSquares(v2 point, float angle) {
    doSomethingToSquares(
        [point, angle](v2 &v) { v.rotate2dPoint(point, angle); });
  }
  void shearSquares(float xShear, float yShear) {
    doSomethingToSquares([xShear, yShear](v2 &v) { v.shear(xShear, yShear); });
  }
  ...
  void setScalingDirection(ScaledDirection sd) { scaleDir = sd; }
  void setShearingDirection(ShearDirection sd) { shearDir = sd; }
  ...
  void animateScaling(float dt) {
    ...
    if (length >= 18)
      scaleDir = ScaledDirection::shrink;
    if (length < 1)
      scaleDir = ScaledDirection::expand;
    ScaledDirection sd = scaleDir;
    doSomethingToSquares([sd](v2 &v) {
      if (sd == ScaledDirection::expand)
        v = v * 1.01;
      if (sd == ScaledDirection::shrink)
        v = v * 0.99;
    });
    if (sd == ScaledDirection::expand)
      length = length * 1.01;
